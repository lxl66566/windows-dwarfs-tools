...
pub fn save_file_dialog(extensions: &[&str], default_filename: &str) -> Option<PathBuf> {
    ...
    if !extensions.is_empty() {
        ...
        // Typically, after the user selects a filter, the dialog automatically handles appending
        // the extension
        let filter_name = if extensions.len() == 1 {
            format!(
                "{} Files (*.{})",
            ...
            // Or separate filter entries can be created for each extension, rfd supports multiple
            // add_filter calls
